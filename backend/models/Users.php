    ...
    public function rules()
    {
        return [
          //  [['first_name', 'last_name','placeofbirth','current_city','current_state','current_country','mobile','dateofbirth','timeofbirth'], 'required'],
            [['username', 'email','timeofbirth','dateofbirth'], 'required'],
            [['status', 'roleId', 'created_at', 'updated_at'], 'integer'],
            [['username', 'password_hash', 'password_reset_token', 'email'], 'string', 'max' => 255],
            [['auth_key'], 'string', 'max' => 32],
            [['username','email'], 'unique'],
            [['password_reset_token'], 'unique'],
        	[['first_name', 'last_name', 'placeofbirth', 'current_city', 'current_state', 'current_country','dateofbirth', 'dasa_at_birth','timeofbirth', 'mobile','password','confirmpassword','dasa_end_date','address','remedies'], 'safe'],
