   ...
   public function actionDynamicdashas()
   {
   	 ...
   	 foreach ($filterArray as $k2 => $v2)
   	 {
	     ...
	     /* date_add($date, date_interval_create_from_date_string($nofDays2.' days')); */
   	 	...
   	 	if($newchild == 6)
   	 	{
   	 		$html .= '<li class="'.$class.' extracls"><span><i class="fa " aria-hidden="true" ><b>'.$k2.'</b></i></span></li>        <li>'.date('Y-m-d',strtotime($finalAry[$p2]['from_date'])).'</li>        <li>'.date('Y-m-d',strtotime($finalAry[$p2]['end_date'])).'</li>';
   	 	}
   	 	else{
   	 	 $html .= '<li class="'.$class.'"><span><i class="fa fa-angle-right arrow parentclick" aria-hidden="true" parentprop="'.$newchild.'" parentdays="'.$nofDays2.'" fromdate="'.$finalAry[$p2]['from_date'].'" id="'.$parentid.'clickchild'.$childno.'parent'.$overall.'" planetname="'.$k2.'"><b>'.$k2.'</b></i></span></li>        <li>'.date('Y-m-d',strtotime($finalAry[$p2]['from_date'])).'</li>        <li>'.date('Y-m-d',strtotime($finalAry[$p2]['end_date'])).'</li> ';
   	 	}
   	 	$p2++;
   	 	//echo $nofDays2;exit();
   	 	$html .='</ul>';
...
//    		$setNotifications = new Notification('Your horoscope today', 'Your daily predictions from SriAstrology and clicking on this should link
// to Daily Predictions page');
   		$setNotifications = new Notification(' Your daily prediction from Sri Astrology', 'Click here to see daily prediction for your horoscope.');
   		$setNotifications->setSound(true);
   		$setNotifications->setSound(true);
   		$notificationextraInfo = array();
   		$notificationextraInfo['screen'] = 'daily';
   		$message = new Message();
